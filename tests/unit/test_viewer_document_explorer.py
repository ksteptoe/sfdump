"""Tests for Document Explorer glob-to-regex conversion."""

from __future__ import annotations

from sfdump.viewer_app.ui.document_explorer import _glob_to_regex


class TestGlobToRegex:
    """Tests for _glob_to_regex function."""

    def test_literal_text_unchanged(self):
        """Plain text without wildcards passes through unchanged."""
        assert _glob_to_regex("PIN010063") == "PIN010063"
        assert _glob_to_regex("SIN002469") == "SIN002469"
        assert _glob_to_regex("Softcat") == "Softcat"

    def test_asterisk_converts_to_dot_star(self):
        """Asterisk (*) converts to regex .* for any characters."""
        assert _glob_to_regex("PIN01006*") == "PIN01006.*"
        assert _glob_to_regex("*Softcat*") == ".*Softcat.*"
        assert _glob_to_regex("SIN*") == "SIN.*"
        assert _glob_to_regex("*") == ".*"

    def test_question_mark_converts_to_dot(self):
        """Question mark (?) converts to regex . for single character."""
        assert _glob_to_regex("PIN01006?") == "PIN01006."
        assert _glob_to_regex("PIN0100??") == "PIN0100.."
        assert _glob_to_regex("?") == "."

    def test_combined_wildcards(self):
        """Mixed wildcards work correctly."""
        assert _glob_to_regex("PIN*?") == "PIN.*."
        assert _glob_to_regex("?IN01006*") == ".IN01006.*"
        assert _glob_to_regex("*?*") == ".*..*"

    def test_character_class_simple(self):
        """Character classes [abc] pass through."""
        assert _glob_to_regex("PIN01006[123]") == "PIN01006[123]"
        assert _glob_to_regex("[abc]") == "[abc]"

    def test_character_class_range(self):
        """Character ranges [1-5] pass through."""
        assert _glob_to_regex("PIN01006[0-9]") == "PIN01006[0-9]"
        assert _glob_to_regex("PIN01006[1-5]") == "PIN01006[1-5]"
        assert _glob_to_regex("[a-z]") == "[a-z]"
        assert _glob_to_regex("[A-Za-z0-9]") == "[A-Za-z0-9]"

    def test_character_class_negation_glob_style(self):
        """Glob-style negation [!abc] converts to regex [^abc]."""
        assert _glob_to_regex("[!0-9]") == "[^0-9]"
        assert _glob_to_regex("PIN[!abc]") == "PIN[^abc]"
    ...
    def test_unclosed_bracket_escaped(self):
        ...
        assert _glob_to_regex("PIN[123") == "PIN\\[123"
        assert _glob_to_regex("[") == "\\["

    def test_regex_special_chars_escaped(self):
        """Regex special characters are escaped for literal matching."""
        assert _glob_to_regex("file.pdf") == "file\\.pdf"
        assert _glob_to_regex("a+b") == "a\\+b"
        assert _glob_to_regex("(test)") == "\\(test\\)"
        assert _glob_to_regex("a|b") == "a\\|b"
        assert _glob_to_regex("end$") == "end\\$"
        assert _glob_to_regex("^start") == "\\^start"
        assert _glob_to_regex("a{2}") == "a\\{2\\}"

    def test_real_world_examples(self):
        """Real-world search patterns work correctly."""
        # Invoice number searches
        assert _glob_to_regex("PIN01006*") == "PIN01006.*"
        assert _glob_to_regex("SIN00246?") == "SIN00246."
        assert _glob_to_regex("PIN0100[6-9]*") == "PIN0100[6-9].*"

        # Filename with extension
        assert _glob_to_regex("*.pdf") == ".*\\.pdf"
        assert _glob_to_regex("invoice*.pdf") == "invoice.*\\.pdf"

        # Company name variations
        assert _glob_to_regex("Arm*") == "Arm.*"
        assert _glob_to_regex("*Limited") == ".*Limited"


class TestGlobToRegexEdgeCases:
    """Edge case tests for _glob_to_regex."""
    ...
    def test_multiple_character_classes(self):
        ...
        assert _glob_to_regex("[a-z][0-9]") == "[a-z][0-9]"
        assert _glob_to_regex("PIN[0-9][0-9][0-9]") == "PIN[0-9][0-9][0-9]"
